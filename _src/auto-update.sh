...
if [[ $novo_boletim = 0 && $commit = 0 ]]; then
    ...
    if [ ${check:0:1} = "s" ]; then
        ...
        if [ $? = 0 ]; then
            ...
            git add projecoes-${hoje}.html _includes/lista_projecoes.html dados.html dados_metadata.html data/dados_de_trabalho.csv data/proj30.csv data/coefs_estimados.csv data_ocr_cor2_metadata.html historico.html index.html planilha_de_trabalho_metadata.html sitemap.xml somar_prev/projecao30_*.csv projecoes_30_dias_metadata.html
