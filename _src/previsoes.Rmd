...
* **Chuva nos últimos 30 dias**: `r round(sum(window(c2.w$pluv,start=max(time(c2.w))-29)),0)` mm.
* **Média histórica dos últimos 30 dias**: `r as.numeric(round(pluv.hist$ph.m[time(pluv.hist)==max(time(c2.w))-29]*30,0))` mm.	
<br>	
* **Todas as projeções estão em percentual do volume total**.
  ...
  * Para converter para saldo percentual do volume morto multiplique por 1,292 e subtraia 29,2.
...
mm de chuva acumulada em  `r format(min(time(ph.next)), "%B")`, 
`r as.numeric(medias$ph.cum[as.yearmon(max(time(ph.next)))])` mm em
`r format(max(time(ph.next)), "%B")` e `r as.numeric(round(pluv.hist$ph.m[time(pluv.hist)==max(time(c2.w))-29]*30,0))` mm
de `r format(min(time(ph.next)), "%d/%B")` a `r format(max(time(ph.next)), "%d/%B")`.
* Nos últimos 30 dias a chuva acumulada foi `r round(sum(window(c2.w$pluv,start=max(time(c2.w))-29)),0)` mm.
