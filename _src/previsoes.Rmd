...
```
Atualizadas diariamente por volta das 13:00 horas
...
kable(tab1,
      ...
      caption=paste("Projeção e seus limites de credibilidade em % do volume total. Chuva prevista de ",
          round(bol.pluv[1],1), " mm/dia e vazão de retirada de ", round(def.max,1), " m3/s.", sep=""  ))
```

<br>


* De acordo com a [SABESP](http://www2.sabesp.com.br/mananciais/DivulgacaoSiteSabesp.aspx),
**o volume armazenado em `r format(max(time(c1)), '%d/%m/%Y')` é
`r as.numeric(round(c1$v.rel2[time(c1)==max(time(c1))],1))` % do volume total**,
que corresponde a `r as.numeric(round(c1$v.rel[time(c1)==max(time(c1))],1))` % do volume útil.
