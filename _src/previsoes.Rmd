...
* **Volume armazenado em `r format(max(time(c1)), '%d/%m/%Y')`** ([SABESP](http://www2.sabesp.com.br/mananciais/DivulgacaoSiteSabesp.aspx)):
    * `r as.numeric(round(c1$v.rel2[time(c1)==max(time(c1))],1))` % do volume disponível
    * o mesmo que `r as.numeric(round(c1$v.rel[time(c1)==max(time(c1))],1))` % do volume útil.
    * o mesmo que `r as.numeric(round(c1$v.rel[time(c1)==max(time(c1))]-29.2,1))` % de saldo do volume morto.
