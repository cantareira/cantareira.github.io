...
##Usando a chuva prevista de `r format(bol.pred$inicio, "%d/%m")` a `r format(bol.pred$fim, "%d/%m/%Y")`
<br>

```{r tabela boletins}
tab1 <-as.data.frame(tab.pred.bol[-1])
rownames(tab1) <- format(time(tab.pred.bol[-1,]), "%d/%m")
kable(tab1,
...
## With google vis
## ```{r tabela boletins, results="asis"}
## tab1 <-as.data.frame(tab.pred.bol[-1])
## colnames(tab1) <- c("Projeção", "Limite inferior", "Limite superior")
## tab1$Data <- format(time(tab.pred.bol[-1,]), "%d/%m")
## print(gvisTable(tab1[,c(4,1:3)],
##                 options(sort="disable"),
##                      formats=list(Data=NULL, "Projeção"="#.#%", "Limite inferior"="#.#%", "Limite superior"="#.#%")),
##                 tag="chart")
## With xtables
## tab1 <-as.data.frame(tab.pred.bol[-1])
## rownames(tab1) <- format(time(tab.pred.bol[-1,]), "%d/%m")
## colnames(tab1) <- c("Projeção", "Limite inferior", "Limite superior")
## print(xtable(tab1, digits=1),
##              type="html")
```
...
### Como são feitas as projeções para 5 dias?

Usamos um modelo matemático para projetar quanta água restará no sistema Cantareira
nos próximos 5 dias a partir de `r format(max(time(c1))+1, "%d/%m/%Y")`.
...
```{rtabela probs}
#<br>
### Quais as chances de o volume diminuir em 30 dias?
#<br>
