...
`r format(max(time(ph.next)), "%B")` e `r round(pluv.hist.30,1)` mm
de `r format(min(time(ph.next)), "%d/%B")` a `r format(max(time(ph.next)), "%d/%B")`.
* Nos últimos 30 dias a chuva acumulada foi `r round(pluv.ac.30,1)` mm.
